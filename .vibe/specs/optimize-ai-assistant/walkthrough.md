...
- **Dynamic Scroll-Reactive Slide Transitions**: Connected the show/hide status of the bottom search/filter bar to the scroll-to-top and assistant buttons using the global `FilterContext`. When the filter bar is shown (e.g. user scrolls down), both bubbles automatically transition smoothly from `bottom-6` to `bottom-20` (`transition-all duration-300`). When the filter bar hides (e.g. user scrolls to the top or bottom limits), both bubbles transition back down to `bottom-6`, maximizing workspace usability.
- **Project Structure Refactoring**: Renamed the components folder `src/components/ai-chat` to `src/components/chat` to make the workspace organization cleaner. Converted the main component wrapper from `ChatWidget.tsx` to `index.tsx` to optimize import statements in [layout.tsx](file:///Users/amrabed/Library/CloudStorage/OneDrive-Personal/code/amrabed.com/src/app/layout.tsx) (now importing from `@/components/chat` directly) and renamed `ChatWidgetClient.tsx` to `client.tsx` for cleaner module segregation. Removed the old `ai-chat` folder entirely.
